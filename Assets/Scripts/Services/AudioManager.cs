...
using UnityEngine;
[CreateAssetMenu(fileName = "AudioManager", menuName = "Service/AudioManager", order = 0)]
public class AudioManager : Service
{
    ...
    public AudioSource PlayMusic(string _music_track_name = "", float volume = 1f, float fadeout_duration = 2f)
    {
        ...
        if (music_track != null)
        {
            ...
            if (already_playing_music) { FadeInSource(music_track, fadeout_duration); }
        }
    ...
    #region Specific sound methods
    [SerializeField]
    FloatRange movement_sound_range = new FloatRange(1f, 1.2f);
    public void MoveSound()
    {
        PlayResource("move", .5f, movement_sound_range);
    }


    IEnumerator IslandSoundStep()
    {
        AudioSource asc = PlayResource("island_hi", .4f, new FloatRange(1f, 1.1f));
        while (false || asc.time < .1f)
        {
            yield return null;
        }
        PlayResource("island_lo", 1f, new FloatRange(1f, 1.1f));
    }

    public void IslandSound()
    {
        StartCoroutine(IslandSoundStep());
    }
